...
export async function clientLogin({
    ...
    if (type === UserRole.ADMIN &&
        userRole !== UserRole.ADMIN) throw new Error("You are not an admin user.");
...
export async function createFirstAdminUser({
    ...
    if (adminUser) throw new Error("An Admin user already exists. Please login and create a new user.");
    try {
